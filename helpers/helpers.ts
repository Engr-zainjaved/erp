...
export const hasNotch = () => {
   ...
   * For storybook test
   */
  const storybook = typeof window !== "undefined" ? window.location !== window.parent.location : "";
  // @ts-ignore
  const iPhone =
...
export function isProductionBranch(branchData: any, branchId: number) {
  if (
    branchData &&
    branchData.data &&
    branchData.data.data &&
    branchData.data.data.Production &&
    branchData.data.data.Production.branches
  ) {
    for (let i = 0; i < branchData.data.data.Production.branches.length; i++) {
      if (branchData.data.data.Production.branches[i].id === branchId) {
        return true;
      }
    }
  }
...
export const delayLoading = async (ms: number) => {
